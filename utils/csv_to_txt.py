...
class Beam:
    ...
    def __post_init__(self):
        ...
        assert not self.phi_col in self.volt_cols  # not the same as phi
        assert not self.theta_cols in self.volt_cols  # not the same as theta
    ...
    def _delete_wrap(self, col=0, period=2*np.pi):
        ...
        """
        wrap_idx=np.argwhere(self.data[:,col] - self.data[:,col].min() >= period)[:,0]  # rows where the angle wraps around
        if len(wrap_idx):  # axis wraps around
            new_data = np.delete(self.data, wrap_idx, axis=0)  # delete the given rows
    ...
    def _shift_phi(self):
        ...
        assert np.diff(new_data[:, self.phi_col]).min() >= 0  # phi is still increasing
        assert new_data[:, self.phi_col].min() >= 0  # and is bigger than 0 everywhere
        return new_data
    ...
    def _process_data(self):
        self.data = _delete_wrap(self.data, col=self.theta_col, period=np.pi)
        self.data = _delete_wrap(self.data, col=self.phi_col, period=2*np.pi)
        self.data = _rev_indices(self.data, self.phi_col, self.theta_col)
        self.data = _shift_phi(self.data, self.phi_col)
   ...
   def efield_to_power(self):
    ...
    def to_txt(self, outpath, to_power=True):
        ...
        if to_power:
            ...
            np.savetxt(outpath, power, header='Theta [rad] Phi [rad] Abs(V) [V] \n\n', comments='')
        else:
